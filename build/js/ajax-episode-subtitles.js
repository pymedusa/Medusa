            ...
            $.getJSON(url, function (data) {
                ...
                if (data.result === 'success') {
                    $.each(data.subtitles, function (index, subtitle) {
                        ...
                        var row = '<tr style="font-size: 95%;">' + '<td style="white-space:nowrap;">' + provider + ' ' + subtitle.provider + '</td>' + '<td>' + flag + '</td>' + '<td title="' + subtitle.sub_score + '/' + subtitle.min_score + '"> ' + subtitleScore + '</td>' + '<td class="tvShow"> ' + subtitleLink + '</td>' + '<td>' + missingGuess + '</td>' + '</tr>';
