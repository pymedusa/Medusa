...
<html>
    ...
    <body data-controller="${controller}" data-action="${action}">
        ...
        % if submenu:
        <div id="SubMenu" class="hidden-print nav-tabs">
            <ul>
            <% first = True %>
                    ...
                    <% icon_class = '' if 'icon' not in menuItem else ' ' + menuItem['icon'] %>
                      % if type(menuItem['path']) == dict:
                          ...
                          % for cur_link in menuItem['path']:
                              ${("&middot; ", "")[bool(inner_first)]}<li><a class="inner" href="${srRoot}/${menuItem['path'][cur_link]}">${cur_link}</a></li>
                              <% inner_first = False %>
                          % endfor
                      % else:
                          <li><a href="${srRoot}/${menuItem['path']}" class="btn${('', ' confirm ' + menuItem.get('class', ''))['confirm' in menuItem]}">${('', '<span class="pull-left ' + icon_class + '"></span> ')[bool(icon_class)]}${menuItem['title']}</a></li>
                          <% first = False %>
                      % endif
                % endif
            % endfor
        ...
        <div id="navSelect" class="pull-left form-inline">
        </div>
