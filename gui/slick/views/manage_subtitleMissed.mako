        ...
        <form action="${srRoot}/manage/downloadSubtitleMissed" method="post">
            ...
            % for cur_indexer_id in sorted_show_ids:
                <tr id="${cur_indexer_id}">
                    ...
                    <th colspan="3" style="text-align: left;"><a class="whitelink" href="#/displayShow?showId=${cur_indexer_id}">${show_names[cur_indexer_id]}</a> (${ep_counts[cur_indexer_id]}) <input type="button" class="pull-right get_more_eps btn" id="${cur_indexer_id}" value="Expand" /></th>
                </tr>
            % endfor
            </table>
        </form>
