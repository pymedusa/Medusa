...
class NameParser(object):
    ...
    def _parse_string(self, name):
        ...
        series = helpers.get_show(result.series_name, self.try_indexers) if not self.naming_pattern else None

        # confirm passed in series object indexer id matches result series object indexer id
        series = None if series and self.series and series.indexerid != self.series.indexerid else series
        result.series = series or self.series

        # if this is a naming pattern test or result doesn't have a series object then return best result
        ...
        # if we have an air-by-date series and the result is air-by-date,
        ...
        if result.series.air_by_date and result.is_air_by_date:
            ...
            sql_result = main_db_con.select(
                ...
                [result.series.indexerid, result.series.indexer, airdate])
            ...
            for episode_number in episode_numbers:
                ...
                if result.series.is_scene:
                    (season, episode) = scene_numbering.get_indexer_numbering(
                        result.series.indexerid,
                        result.series.indexer,
                        season_number,
                        episode_number,
                    )
                    log.debug(
            ...
            for absolute_episode in result.ab_episode_numbers:
                ...
                if result.series.is_scene:
                    a = scene_numbering.get_indexer_absolute_numbering(result.series.indexerid,
                                                                       result.series.indexer, absolute_episode,
                                                                       True, scene_season)
