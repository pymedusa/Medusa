...
class Notifier(object):
    ...
    def _send_update_library(self, host, show_name=None):
        ...
        if kodi_api:
            ...
            if not update(host, show_name) and app.KODI_UPDATE_FULL:
    ...
    def _update_library(self, host=None, show_name=None):  # pylint: disable=too-many-return-statements, too-many-branches
        ...
        if show_name:
            ...
            log.debug(u'Updating library in KODI via JSON method for show {0}', show_name)
            ...
            # get tvshowid by show_name
            show_response = self._send_to_kodi(shows_command, host)

            if show_response and 'result' in show_response and 'tvshows' in show_response['result']:
                shows = show_response['result']['tvshows']
            else:
            ...
            log.debug(u'Received Show: {0} with ID: {1} Path: {2}', show_name, tvshowid, path)

            if not path:
                log.warning(u'No valid path found for {0} with ID: {1} on {2}', show_name, tvshowid, host)
                return False

            log.debug(u'KODI Updating {0} on {1} at {2}', show_name, host, path)
            update_command = {
                'jsonrpc': '2.0',
            ...
            request = self._send_to_kodi(update_command, host)
            if not request:
                log.warning(u'Update of show directory failed on {0} on {1} at {2}', show_name, host, path)
                return False
