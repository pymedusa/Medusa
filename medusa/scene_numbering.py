...
def get_scene_numbering(show_obj, season, episode, fallback_to_xem=True):
    """
    ...
    """
    if show_obj is None or season is None or episode is None:
        return season, episode

    if show_obj and not show_obj.is_scene:
        return season, episode

    result = find_scene_numbering(show_obj, season, episode)
    if result:
        return result
    else:
        if fallback_to_xem:
            xem_result = find_xem_numbering(show_obj, season, episode)
            if xem_result:
                return xem_result
        return season, episode


def find_scene_numbering(show_obj, season, episode):
    """
    ...
    rows = main_db_con.select(
        ...
        [show_obj.indexer, show_obj.show_id, season, episode])

    if rows:
        return int(rows[0][b"scene_season"]), int(rows[0][b"scene_episode"])


def get_scene_absolute_numbering(show_obj, absolute_number, fallback_to_xem=True):
    """
    Returns a tuple, (season, episode), with the scene numbering (if there is one),
    otherwise returns the xem numbering (if fallback_to_xem is set), otherwise
    returns the TVDB numbering.
    ...
    :return: (int, int) a tuple with (season, episode)
    """
    if show_obj is None or absolute_number is None:
        return absolute_number

    if show_obj and not show_obj.is_scene:
        return absolute_number

    result = find_scene_absolute_numbering(show_obj, absolute_number)
    if result:
        return result
    else:
        if fallback_to_xem:
            xem_result = find_xem_absolute_numbering(show_obj, absolute_number)
            if xem_result:
                return xem_result
        return absolute_number


def find_scene_absolute_numbering(show_obj, absolute_number):
    """
    Same as get_scene_numbering(), but returns None if scene numbering is not set
    """
    if show_obj is None or absolute_number is None:
    ...
    rows = main_db_con.select(
        ...
        [show_obj.indexer, show_obj.show_id, absolute_number])

    if rows:
        return int(rows[0][b"scene_absolute_number"])


def get_indexer_numbering(show_obj, sceneSeason, sceneEpisode, fallback_to_xem=True):
    """
    ...
    """
    if show_obj is None or sceneSeason is None or sceneEpisode is None:
    ...
    rows = main_db_con.select(
        ...
        [show_obj.indexer, show_obj.show_id, sceneSeason, sceneEpisode]
    )
    ...
    if rows:
        ...
        if fallback_to_xem:
            return get_indexer_numbering_for_xem(show_obj, sceneSeason, sceneEpisode)
        return sceneSeason, sceneEpisode


def get_indexer_absolute_numbering(show_obj, sceneAbsoluteNumber, fallback_to_xem=True, scene_season=None):
    """
    ...
    """
    if show_obj is None or sceneAbsoluteNumber is None:
    ...
    if scene_season is None:
        rows = main_db_con.select(
            ...
            [show_obj.indexer, show_obj.show_id, sceneAbsoluteNumber])
    else:
        rows = main_db_con.select(
            "SELECT absolute_number FROM scene_numbering WHERE indexer = ? and indexer_id = ? and scene_absolute_number = ? and scene_season = ?",
            [show_obj.indexer, show_obj.show_id, sceneAbsoluteNumber, scene_season])

    if rows:
        return int(rows[0][b"absolute_number"])
    else:
        if fallback_to_xem:
            return get_indexer_absolute_numbering_for_xem(show_obj, sceneAbsoluteNumber, scene_season)
        return sceneAbsoluteNumber


def set_scene_numbering(show_obj, season=None, episode=None,  # pylint:disable=too-many-arguments
    ...
    if season is not None and episode is not None and absolute_number is None:
        ...
        main_db_con.action(
            ...
            [sceneSeason, sceneEpisode, show_obj.indexer, show_obj.show_id, season, episode])
    # absolute_number can be 0 so can't check "if absolute_number"
        ...
        main_db_con.action(
            ...
            [sceneAbsolute, show_obj.indexer, show_obj.show_id, absolute_number])

    show_obj.flush_episodes()
    show_obj.erase_cached_parse()


def find_xem_numbering(show_obj, season, episode):
    """
...
def find_xem_absolute_numbering(show_obj, absolute_number):
    """
    Returns the scene numbering, as retrieved from xem.
    Refreshes/Loads as needed.
...
def get_indexer_numbering_for_xem(show_obj, sceneSeason, sceneEpisode):
    """
...
def get_indexer_absolute_numbering_for_xem(show_obj, sceneAbsoluteNumber, scene_season=None):
    """
    Reverse of find_xem_numbering: lookup a tvdb season and episode using scene numbering
...
def get_scene_numbering_for_show(show_obj):
    """
...
def get_xem_numbering_for_show(show_obj, refresh_data=True):
    """
    Returns a dict of (season, episode) : (sceneSeason, sceneEpisode) mappings
    for an entire show.  Both the keys and values of the dict are tuples.
...
def get_scene_absolute_numbering_for_show(show_obj):
    """
    Returns a dict of (season, episode) : (sceneSeason, sceneEpisode) mappings
    for an entire show.  Both the keys and values of the dict are tuples.
    Will be empty if there are no scene numbers set
...
def get_xem_absolute_numbering_for_show(show_obj):
    """
    Returns a dict of (season, episode) : (sceneSeason, sceneEpisode) mappings
    for an entire show.  Both the keys and values of the dict are tuples.
    Will be empty if there are no scene numbers set in xem
...
def xem_refresh(show_obj, force=False):
    """
    Refresh data from xem for a tv show

    :param indexer_id: int
    """
    if not show_obj or show_obj.show_id < 1:
        return
    ...
    if refresh or force:
        logger.log(
            u'Looking up XEM scene mapping for show ID {0} on {1}'.format(show_id, show_obj.indexer_name), logger.DEBUG)

        # mark refreshed
        main_db_con.upsert(
            "xem_refresh",
            {'last_refreshed': int(time.mktime(datetime.datetime.today().timetuple()))},
            {'indexer': indexer_id, 'indexer_id': show_id}
        )
        ...
        try:
            ...
            if not parsed_json or 'result' not in parsed_json or 'success' not in parsed_json['result'] or 'data' not in parsed_json or str(show_id) not in parsed_json['data']:
                logger.log(u'No XEM data for show ID {0} on {1}'.format(show_id, show_obj.indexer_name), logger.DEBUG)
                return

            # XEM API URL
            url = "http://thexem.de/map/all?id={0}&origin={1}&destination=scene".format(show_id, indexerApi(indexer_id).config['xem_origin'])
            parsed_json = safe_session.get_json(url)
            if not parsed_json or 'result' not in parsed_json or 'success' not in parsed_json['result']:
                logger.log(u'No XEM data for show ID {0} on {1}'.format(indexer_id, show_obj.indexer_name), logger.DEBUG)
                return
                ...
                if 'scene' in entry:
                    cl.append([
                        ...
                        [entry['scene']['season'], entry['scene']['episode'],
                         entry['scene']['absolute'], indexer_id, show_id,
                    ...
                    cl.append([
                        ...
                        [entry[indexerApi(indexer_id).config['xem_origin']]['absolute'], indexer_id, show_id,
                         entry[indexerApi(indexer_id).config['xem_origin']]['season'],
                         entry[indexerApi(indexer_id).config['xem_origin']]['episode']]
                    ])
                if 'scene_2' in entry:  # for doubles
                    cl.append([
                        ...
                        [entry['scene_2']['season'], entry['scene_2']['episode'],
                         entry['scene_2']['absolute'], indexer_id, show_id,
                         entry[indexerApi(indexer_id).config['xem_origin']]['season'],
                         entry[indexerApi(indexer_id).config['xem_origin']]['episode']]
                    ])
            ...
            logger.log(u"Exception while refreshing XEM data for show ID {0} on {1}: {2}".format
                       (show_id, show_obj.indexer_name, ex(e)), logger.WARNING)
            logger.log(traceback.format_exc(), logger.DEBUG)


def fix_xem_numbering(show_obj):  # pylint:disable=too-many-locals, too-many-branches, too-many-statements
    """
    Returns a dict of (season, episode) : (sceneSeason, sceneEpisode) mappings
    for an entire show.  Both the keys and values of the dict are tuples.
    Will be empty if there are no scene numbers set in xem
    ...
    logger.log(
        u'Fixing any XEM scene mapping issues for show ID {0} on {1}'.format(show_obj.show_id, show_obj.indexer_name),
        logger.DEBUG)
    ...
    for row in rows:
        ...
        if update_absolute_number:
            cl.append([
                ...
                [absolute_number, show_obj.indexer, show_obj.show_id, season, episode]
            ])
            update_absolute_number = False
        ...
        if update_scene_season:
            cl.append([
                ...
                [scene_season, show_obj.indexer, show_obj.show_id, season, episode]
            ])
            update_scene_season = False
        ...
        if update_scene_episode:
            cl.append([
                ...
                [scene_episode, show_obj.indexer, show_obj.show_id, season, episode]
            ])
            update_scene_episode = False
        ...
        if update_scene_absolute_number:
            cl.append([
                ...
                [scene_absolute_number, show_obj.indexer, show_obj.show_id, season, episode]
            ])
            update_scene_absolute_number = False
