...
class SeriesHandler(BaseRequestHandler):
    ...
    series_slug_pattern = re.compile(r'\w+[1-9][\d]+$')
    ...
    def get(self, series_slug, path_param=None):
        ...
        if not self.series_slug_pattern.match(series_slug):
            resource = series_slug
            if resource == 'existingSeries':
                return self._existing_series_data()

            return self._bad_request("Invalid identifier '{0}'".format(resource))
