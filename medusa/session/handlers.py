...
def cloudflare(session, resp, **kwargs):
    ...
    protection.
    """
    cf = Cloudflare(CloudScraper())
    if cf.is_IUAM_Challenge(resp) or cf.is_Captcha_Challenge(resp):
        ...
        if (cf.is_New_IUAM_Challenge(resp) or cf.is_Firewall_Blocked(resp)
                or cf.is_New_Captcha_Challenge(resp)):
