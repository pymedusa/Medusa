...
class Series(TV):
    ...
    def want_season(self, season, quality, forced_search=False, download_current_quality=False):
        """Check if we we want the episodes included withing the multi-ep search result."""
        ...
        if not Quality.wanted_quality(quality, allowed_qualities, preferred_qualities):
            log.debug(
                u"{id}: Ignoring found result for '{show}' S{season} with unwanted quality '{quality}'", {
                    'id': self.indexerid,
                    'show': self.name,
                    'season': season,
        ...
        sql_results = main_db_con.select(
            ...
            b'WHERE '
            b'  showid = ? '
            b'  AND season = ? ', [self.indexerid, season])

        if not sql_results or not len(sql_results):
            log.debug(
                u'{id}: Unable to find a episodes matching the season searched in database.'
                u' Ignoring found result for {show} S{season} with quality {quality}', {
                    'id': self.indexerid,
                    'show': self.name,
                    'season': season,
                    'quality': Quality.qualityStrings[quality],
        ...
        for episode in sql_results:
            ...
            ep_status = int(episode[b'status'])
            ep_status_text = statusStrings[ep_status].upper()
            ep_number = int(episode[b'episode'])
            # manually_searched = episode[b'manually_searched']
            _, cur_quality = Quality.split_composite_status(ep_status)

            # if it's one of these then we want it as long as it's in our allowed initial qualities
            if ep_status == WANTED:
                wanted_episodes.append(ep_number)
                log.debug(
                    u"{id}: '{show}' {ep} status is 'WANTED'. Accepting result with quality '{new_quality}'", {
                        'id': self.indexerid,
                        'status': ep_status_text,
                        'show': self.name,
                        'ep': episode_num(season, ep_number),
                        'new_quality': Quality.qualityStrings[quality],
                    }
                )
            #
            # should_replace = Quality.should_replace(ep_status, cur_quality, quality, allowed_qualities,
            ...
            #         'new_quality': Quality.qualityStrings[quality],
            #         'reason': should_replace[ep_number][1],
            #     }
            # )
