...
from anidb_exceptions import (anidb_error, anidb_shownotfound, anidb_showincomplete,
                               anidb_seasonnotfound, anidb_episodenotfound, anidb_attributenotfound)
from anidb_ui import BaseUI, ConsoleUI
...
class Anidb_API(object):
    ...
    def __init__(self,  # pylint: disable=too-many-locals,too-many-arguments
        ...
        if self.config['debug_enabled']:
            warnings.warn('The debug argument to anidb_api.__init__ will be removed in the next version. '
                          'To enable debug messages, use the following code before importing: '
                          'import logging; logging.basicConfig(level=logging.DEBUG)')
            logging.basicConfig(level=logging.DEBUG)

        # List of language from http://theanidb.com/api/0629B785CE550C8D/languages.xml
        # Hard-coded here as it is realtively static, and saves another HTTP request, as
        # recommended on http://theanidb.com/wiki/index.php/API:languages.xml
        ...
        # Initiate the simpleanidb API
        self.API = Anidb(self.config['session'], use_tmp_cache=True)
    ...
    def _getTempDir(self):  # pylint: disable=no-self-use
        ...
        if hasattr(os, 'getuid'):
            ...
            try:
                ...
                return os.path.join(tempfile.gettempdir(), 'anidb_api')
    ...
    def _show_search(self, show):
        """
        Uses the pyanidb API to search for a show
    ...
    def search(self, series):
        """This searches anidb.com for the series name
    ...
    def _get_show_by_id(self, anidb_id=None):  # pylint: disable=unused-argument
        """
        Retrieve anidb show information by anidb id, or if no anidb id provided by passed external id.

        :param anidb_id: The shows anidb id
    ...
    def _get_episode_list(self, maze_id, specials=False):  # pylint: disable=unused-argument
        """
        Get all the episodes for a show by anidb id

        :param maze_id: Series anidb id.
    ...
    def _getSeries(self, series):
        ...
        if not allSeries:
            ...
            raise anidb_shownotfound('Show search returned zero results (cannot find show on TVDB)')
    ...
    def _getShowData(self, sid, language, getEpInfo=False):  # pylint: disable=too-many-branches,too-many-statements,too-many-locals
        """Takes a series ID, gets the epInfo URL and parses the anidb json response
        ...
        if self.config['language'] is None:
            ...
            if language is None:
                raise anidb_error("config['language'] was None, this should not happen")
        ...
        if not seriesInfoEt:
            log().debug('Series result returned zero')
            raise anidb_error('Series result returned zero')
        ...
        if getEpInfo:
            ...
            if not epsEt:
                ...
                raise anidb_showincomplete('Show search returned incomplete results (cannot find complete show on anidb)')
