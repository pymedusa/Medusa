        ...
        with BS4Parser(data, 'html5lib') as html:
            ...
            for result in torrent_rows:
                ...
                try:
                    ...
                    time_raw = remove_strings(cells[1].get_text(), ["+", " - in TV shows", " ago"]).replace("Last", "1").replace("Yesterday", "1 day") if cells[1] else None
                    time = 0 if time_raw and time_raw == 'right now' else pytimeparse.parse(time_raw) if time_raw else None
                    pubdate = (datetime.datetime.now() - datetime.timedelta(seconds=time)).replace(second=0, microsecond=0) if time else None
