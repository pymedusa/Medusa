...
class ShowUpdater(object):  # pylint: disable=too-few-public-methods
    ...
    def run(self, force=False):  # pylint: disable=unused-argument, too-many-locals, too-many-branches, too-many-statements
        ...
        if not data:
            logger.log(u"Could not get the recently updated show data from {0}. Retrying later. Url was: {1}".format(sickbeard.indexerApi(INDEXER_TVDB).name, url))
            self.amActive = False
            return
        ...
        for cur_show in sickbeard.showList:
            ...
            if cur_show.indexer in bad_indexer:
                logger.log(u"Indexer is no longer available for show [ {0} ] ".format(cur_show.name), logger.WARNING)
            else:
