...
class TVShow(object):  # pylint: disable=too-many-instance-attributes, too-many-public-methods
    ...
    def loadEpisodesFromDir(self):
        ...
        logger.log(u"{0}: Found files: {1}".format(self.indexerid, mediaFiles), logger.DEBUG)
            ...
            if curEpisode is not None:
                if self.subtitles:
                    try:
                        ...
                        logger.log(u"{0}: Could not refresh subtitles".format(self.indexerid), logger.ERROR)
    ...
    def loadEpisodesFromDB(self):  # pylint: disable=too-many-locals
        ...
        try:
            ...
            logger.log(u"Could not load episodes from the DB. Error: {0}".format(error), logger.ERROR)
            return scannedEps
        ...
        for curResult in sql_results:
            ...
            logger.log(u"{0}: Loading {1} episodes from DB".format(curShowid, curShowName), logger.DEBUG)
            deleteEp = False
            ...
            if curSeason not in cachedSeasons:
                try:
                    ...
                    logger.log(u"{0}: {1} (unaired/deleted) in the indexer {2} for {3}. Removing existing records from database".format(curShowid, error.message, sickbeard.indexerApi(self.indexer).name, curShowName), logger.DEBUG)
                    deleteEp = True
    ...
    def deleteShow(self, full=False):
        ...
        for cache_file in ek(glob.glob, ek(os.path.join, image_cache_dir, str(self.indexerid) + '.*')):
            logger.log(u'Attempt to {0} cache file {1}'.format(action, cache_file))
            try:
                ...
                logger.log(u'Unable to {0} {1}: {2} / {3}'.format(action, cache_file, repr(e), str(e)), logger.WARNING)

        # remove entire show folder
        if full:
            try:
                logger.log(u'Attempt to {0} show folder {1}'.format(action, self._location))
                # check first the read-only attribute
                file_attribute = ek(os.stat, self.location)[0]
                if not file_attribute & stat.S_IWRITE:
                    # File is read-only, so make it writeable
                    logger.log(u'Attempting to make writeable the read only folder {0}'.format(self._location), logger.DEBUG)
                    try:
                        ek(os.chmod, self.location, stat.S_IWRITE)
                    except Exception:
                        logger.log(u'Unable to change permissions of {0}'.format(self._location), logger.WARNING)
                ...
                logger.log(u'{0} show folder {1}'.format(('Deleted', 'Trashed')[sickbeard.TRASH_REMOVE_SHOW],
                            self._location))

            except ShowDirectoryNotFoundException:
                logger.log(u"Show folder does not exist, no need to {0} {1}".format(action, self._location), logger.WARNING)
            except OSError as e:
                logger.log(u'Unable to {0} {1}: {2} / {3}'.format(action, self._location, repr(e), str(e)), logger.WARNING)
    ...
    def download_subtitles(self, force=False):
        ...
        logger.log(u"{0}: Downloading subtitles".format(self.indexerid), logger.DEBUG)

        try:
            episodes = self.getAllEpisodes(has_location=True)
            if not episodes:
                logger.log(u"{0}: No episodes to download subtitles for {1}".format(self.indexerid, self.name), logger.DEBUG)
                return
            ...
            logger.log(u"{0}: Error occurred when downloading subtitles for {1}".format(self.indexerid, self.name), logger.DEBUG)
    ...
    def saveToDB(self, forceSave=False):
        ...
        logger.log(u"{0:d}: Saving to database: {1}".format(self.indexerid, self.name), logger.DEBUG)
    ...
    def wantEpisode(self, season, episode, quality, manualSearch=False, downCurQuality=False):  # pylint: disable=too-many-return-statements, too-many-arguments
        ...
        logger.log(u"Any,Best = [ {0} ] [ {1} ] Found = [ {2} ]".format(self.qualitiesToString(allowed_qualities),
    ...
    def getOverview(self, epStatus):  # pylint: disable=too-many-return-statements, too-many-branches
        ...
        if ep_status == WANTED:
            ...
            logger.log(u'Could not parse episode status into a valid overview status: {0}'.format(epStatus), logger.ERROR)
...
class TVEpisode(object):  # pylint: disable=too-many-instance-attributes, too-many-public-methods
    ...
    def loadFromIndexer(self, season=None, episode=None, cache=True, tvapi=None, cachedSeason=None):  # pylint: disable=too-many-arguments, too-many-branches, too-many-statements
        ...
        if not ek(os.path.isdir, self.show._location) and not sickbeard.CREATE_MISSING_SHOW_DIRS and not sickbeard.ADD_SHOWS_WO_DIR:  # pylint: disable=protected-access
            logger.log(u"The show dir {0} is missing, not bothering to change the episode statuses since it'd probably be invalid".format(self.show._location))  # pylint: disable=protected-access
            return
        ...
        if not ek(os.path.isfile, self.location):
            if self.airdate >= datetime.date.today() or self.airdate == datetime.date.fromordinal(1):
                logger.log(u"{0}: Episode airs in the future or has no airdate, marking it {1}".format(self.show.indexerid, statusStrings[UNAIRED]), logger.DEBUG)
                self.status = UNAIRED
            elif self.status in [UNAIRED, UNKNOWN]:
                # Only do UNAIRED/UNKNOWN, it could already be snatched/ignored/skipped, or downloaded/archived to disconnected media
                logger.log(u"Episode has already aired, marking it {0}".format(statusStrings[self.show.default_ep_status]), logger.DEBUG)
                self.status = self.show.default_ep_status if self.season > 0 else SKIPPED  # auto-skip specials
            else:
                logger.log(u"Not touching status [ {0} ] It could be skipped/ignored/snatched/archived".format(statusStrings[self.status]), logger.DEBUG)
    ...
    def get_sql(self, forceSave=False):
        ...
        try:
            ...
            logger.log(u"Error while updating database: {0}".format(
                       (repr(e))), logger.ERROR)
