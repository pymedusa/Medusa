...
const newShow = () => {
    ...
    const searchIndexers = () => {
        ...
        $('#searchResults').empty().html('<img id="searchingAnim" src="images/loading32' + state.components.themeSpinner + '.gif" height="32" width="32" /> searching ' + searchingFor + '...');
            ...
            if (data.results.length === 0) {
                ...
                $.each(data.results, (index, obj) => {
                    ...
                    if (data.langid && data.langid !== '' && obj[1] === 1) { // For now only add the language id to the tvdb url, as the others might have different routes.
                        resultStr += '<a href="' + state.config.anonRedirect + obj[2] + obj[3] + '&lid=' + data.langid + '" onclick="window.open(this.href, \'_blank\'); return false;" ><b>' + obj[4] + '</b></a>';
                    } else {
                        resultStr += '<a href="' + state.config.anonRedirect + obj[2] + obj[3] + '" onclick="window.open(this.href, \'_blank\'); return false;" ><b>' + obj[4] + '</b></a>';
                    }
