...
const init = () => {
    ...
    $('#configForm').ajaxForm({
        beforeSubmit() {
            $('.config_submitter .config_submitter_refresh').each(function() {
                ...
                $(this).after('<span><img src="images/loading16' + state.components.themeSpinner + '.gif"> Saving...</span>');
                $(this).hide();
            });
        },
        success() {
    ...
    $('#create_access_token').popover({
        ...
        content: '<p>Copy the generated token and paste it in the token input box.</p>' +
            '<p><a href="' + state.config.anonRedirect + 'https://github.com/settings/tokens/new?description=Medusa&scopes=user,gist,public_repo" target="_blank">' +
            '<input class="btn" type="button" value="Continue to Github..."></a></p><br/>'
    });

    $('#manage_tokens').on('click', () => {
        window.open(state.config.anonRedirect + 'https://github.com/settings/tokens', '_blank');
    });
};
