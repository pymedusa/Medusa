...
const subtitles = () => {
    ...
    $.fn.addService = function(id, name, url, key, isDefault, showService) { // eslint-disable-line max-params
        ...
        if ($('#service_order_list > #' + id).length === 0 && showService !== false) {
            const toAdd = '<li class="ui-state-default" id="' + id + '"> <input type="checkbox" id="enable_' + id + '" class="service_enabler" CHECKED> <a href="' + state.config.anonRedirect + url + '" class="imgLink" target="_new"><img src="images/services/newznab.gif" alt="' + name + '" width="16" height="16"></a> ' + name + '</li>';
