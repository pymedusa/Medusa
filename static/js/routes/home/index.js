...
const displayShow = require('./display-show');
const editShow = require('./edit-show');
const postProcess = require('./post-process');
const restart = require('./restart');
const snatchSelection = require('./snatch-selection');
...
const index = () => {
    ...
    $('.show-grid').imagesLoaded(() => {
        ...
        $('.show-grid').show().isotope({
            ...
            sortBy: state.config.posterSortby,
            sortAscending: state.config.posterSortdir,
            layoutMode: 'masonry',
            masonry: {
                isFitWidth: true
            },
            getSortData: {
                name(itemElem) {
                    ...
                    return (state.config.sortArticle ? name : name.replace(/^((?:The|A|An)\s)/i, '')).toLowerCase();
                },
    ...
    $('#popover').popover({
        ...
        if (state.config.animeSplitHome) {
    ...
    const rootDir = state.config.rootDirs;
    const rootDirIndex = state.config.selectedRootIndex;
    if (rootDir) {
