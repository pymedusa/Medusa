...
const manageSearches = () => {
    ...
    const updateExceptionTable = exceptions => {
        ...
        const table = $('<ul class="simpleList"></ul>')
            .append(`
                <li>
                    <a href="${state.config.anonRedirect}https://github.com/pymedusa/Medusa/wiki/Scene-exceptions-and-numbering">
            ...
            .append(`
                <li>
                    <a href="${state.config.anonRedirect}http://thexem.de">
    ...
    const updateSpinner = (spinnerContainer, message, showSpinner) => {
        if (showSpinner) {
            message = `<img id="searchingAnim" src="images/loading32${state.components.themeSpinner}.gif" height="16" width="16" />&nbsp;${message}`;
        }
