...
const index = () => {
    if (state.config.layout.schedule === 'list') {
        const sortCodes = {
            date: 0,
            show: 2,
            network: 5
        };
        const sort = state.config.comingEpsSort;
    ...
    if (state.config.layout.schedule in ['banner', 'poster']) {
        $.ajaxEpSearch({
            size: 16,
            loadingImage: 'loading16' + state.components.themeSpinner + '.gif'
        });
        $('.ep_summary').hide();
