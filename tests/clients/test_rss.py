...
def test_add_result_to_feed():
    ...
    result_et = ET.fromstring(rss_raw_text)
    expected_et = ET.fromstring(EXPECTED_RAW_TEXT)

    assert all(elements_equal(e1, e2) for e1, e2 in zip(result_et, expected_et))
...
def elements_equal(elem1, elem2):
    ...
    if len(elem1) != len(elem2):
        return False
    return all(elements_equal(e1, e2) for e1, e2 in zip(elem1, elem2))
