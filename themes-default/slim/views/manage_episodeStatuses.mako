...
% else:
    ...
    <table class="defaultTable manageTable" cellspacing="1" border="0" cellpadding="0">
        ...
        <tr id="${show_id}">
            <th><input type="checkbox" class="allCheck" data-indexer-id="${cur_show[0]}" data-show-id="${cur_show[1]}" id="allCheck-${show_id}" name="${show_id}-all" checked="checked" /></th>
            <th colspan="2" style="width: 100%; text-align: left;"><app-link indexer-id="${cur_show[0]}" class="whitelink" href="home/displayShow?indexername=indexer-to-name&showid=${cur_show[1]}">${show_names[(cur_show[0], cur_show[1])]}</app-link> (${ep_counts[(cur_show[0], cur_show[1])]})
            <input type="button" data-indexer-id="${cur_show[0]}" data-show-id="${cur_show[1]}" class="pull-right get_more_eps btn-medusa" id="${show_id}" value="Expand" /></th>
        </tr>
        % endfor
