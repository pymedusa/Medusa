    ...
    Vue.component('name-pattern', {
        ...
        methods: {
            ...
            async testNaming(pattern, isMulti, animeType) {
                console.log('Test pattern ' + pattern + ' for ' + (isMulti) ? 'multi' : 'single' + ' ep');
                const data = {
                    pattern: pattern,
                    multi: isMulti,
                    anime_type: animeType
                }
                const response = await api.get('config/postProcessing/testNaming', data);
                console.log(response);
                return response;
            },
            updatePatternSamples() {
                // Update single
                const namingResult = this.testNaming(this.pattern, false, this.animeNamingType);

                // Update multi if needed
                if (this.isMulti) {
                    const namingMultiResult = this.testNaming(this.pattern, this.isMulti, this.animeNamingType);
                    debugger;
                }
            }
        },
