        ...
        searchRequestXhr = $.ajax({
            ...
            if (data.results.length === 0) {
                ...
                $.each(data.results, function(index, obj) {
                    ...
                    var whichSeries = obj.indexerName + '|' + obj.indexer + '|' + obj.showUrl + '|' + obj.seriesId + '|' + obj.seriesName + '|' + obj.firstAired + '|' + obj.network;
                    ...
                    if (data.langid && data.langid !== '' && obj.indexer === 1) { // For now only add the language id to the tvdb url, as the others might have different routes.
                        resultStr += '<a href="' + MEDUSA.config.anonRedirect + obj.showUrl + obj.seriesId + '&lid=' + data.langid + '" onclick="window.open(this.href, \'_blank\'); return false;" ><b>' + obj.seriesName + '</b></a>';
                    } else {
                        resultStr += '<a href="' + MEDUSA.config.anonRedirect + obj.showUrl + obj.seriesId + '" onclick="window.open(this.href, \'_blank\'); return false;" ><b>' + obj.seriesName + '</b></a>';
                    }

                    if (obj.firstAired !== null) {
                        var startDate = new Date(obj.firstAired);
                        var today = new Date();
                        if (startDate > today) {
